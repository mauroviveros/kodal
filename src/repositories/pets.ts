...
export const deletePet = async (supabase: SupabaseClient<Database>, { id }: { id: string }) => {
...
export const getPetByMedalId = async (supabase: SupabaseClient<Database>, { medal_id }: { medal_id: string }) => {
  const { data, error } = await supabase.from('pets').select('*').eq('medal_id', medal_id).maybeSingle();
